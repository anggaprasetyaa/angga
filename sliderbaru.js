...
(function($, w, d) {
	var sc = s3slider_config,
		...
		skeleton = '<div class="s3slider" id="s3slider" style="width:' + sc.slider.width + '%;height:' + sc.slider.height + '%;"><ul class="s3slider-content">',
	...
	w.blogger_s3Slider = function(json) {
		...
		for (var i = 0, ien = entry.length; i < ien; ++i) {
			...
			skeleton += '<li><img src="' + image + '" style="display:block;width:' + sc.slider.width + '%;min-height:' + sc.slider.height + '%;height:auto;"><span class="s3slider-caption"><span class="s3slider-title"><a href="' + link + '"' + (sc.newTabLink ? ' target="_blank"' : "") + '>' + title + '</a></span><span class="s3slider-meta"><span class="s3slider-meta-date">' + dd + ' ' + sc.monthArray[parseInt(dm, 10) - 1] + ' ' + dy + '</span><span class="s3slider-meta-comment">' + comment + '</span></span></span></li>';
		}
		skeleton += '</ul></div>';
